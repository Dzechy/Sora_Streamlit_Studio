...
RPM_TIERS = {"Free (unsupported)": 0, "Tier 1": 5, "Tier 2": 10, "Tier 3": 25, "Tier 4": 40, "Tier 5": 75}

def ensure_dir(p: Path) -> Path:
    p.mkdir(parents=True, exist_ok=True); return p

def write_bytes(path: Path, data: bytes) -> Path:
    ensure_dir(path.parent); path.write_bytes(data); return path

def pil_save_image(b64_png: str, out: Path, size: Tuple[int, int] | None = (720, 1280)) -> Optional[Path]:
    try:
        raw = base64.b64decode(b64_png)
        if Image is None: return write_bytes(out, raw)
...
class RateLimiter:
    def __init__(self, rpm: int):
        self.rpm = max(0, int(rpm)); self.tokens = self.rpm
        self.lock = threading.Lock(); self.last_refill = time.monotonic()
    def _maybe_refill(self):
        now = time.monotonic()
        if now - self.last_refill >= 60.0:
            self.tokens = self.rpm; self.last_refill = now
...
def init_openai_client(api_key: Optional[str]) -> Optional[OpenAI]:
    if OpenAI is None: st.error("OpenAI SDK not installed. Try: `pip install openai`"); return None
    key = api_key or os.getenv("OPENAI_API_KEY", ""); 
    if not key: st.warning("Provide an OpenAI API key in the sidebar."); return None
    os.environ["OPENAI_API_KEY"] = key
    try: return OpenAI()
    except Exception as e: st.error(f"Failed to init OpenAI client: {e}"); return None

def _classify_error(e: Exception) -> Dict[str, Any]:
    s = str(e); status=None
    if hasattr(e, "status_code"): status = getattr(e, "status_code")
    elif "Error code:" in s:
        try: status = int(s.split("Error code:")[1].split("-")[0].strip())
        ...
        try: return func()
            ...
            if info["is_quota"] or info["status"]==429:
                if i<attempts-1 and not info["is_quota"]:
                    time.sleep(min(max_delay, base_delay*(2**i))); last_exc=e; continue
                st.error("Rate-limit or quota error from API. Details: "+info['text']); raise
...
def call_api_with_rl(fn: Callable[[], Any]) -> Any:
    rl_acquire(1); return with_retries(fn, attempts=2)
...
def poll_and_download_video(client: OpenAI, video, out_dir: Path, name_prefix: str, seconds: int, model: str) -> Tuple[Optional[Path], Optional[str]]:
    status=getattr(video,"status",None); progress=getattr(video,"progress",0) or 0
    ph=st.empty(); bar=st.progress(0)
    while status not in ("completed","failed","cancelled"):
        progress=getattr(video,"progress",progress) or progress
        status=getattr(video,"status",status); bar.progress(int(max(0,min(100,progress))))
        ph.info(f"Status: {status} • {progress}%"); time.sleep(1.0)
        try: status=getattr(video,"status",status)
        except Exception: pass
    if getattr(video,"status","failed")=="failed":
        err=getattr(getattr(video,"error",None),"message","Video generation failed"); ph.error(err); return None, None
    ...
    try:
        ...
        content.write_to_file(out_path); ph.success(f"Saved: {out_path.name}")
        add_spend_if_new(vid_id, seconds, model)
        return out_path, vid_id
    except Exception as e:
        ph.error(f"Download failed: {e}"); return None, None
...
def enhance_prompt(client: OpenAI, prompt: str, style: str = "director") -> str:
    ...
    try:
        ...
        st.warning(f"Enhancement failed, using original prompt. ({e})"); return prompt
...
def make_reference_image(client: OpenAI, text: str, out_dir: Path, name: str) -> Optional[Path]:
    try:
        ...
        b64=None
        for path in [("output",0,"content",0,"image_base64"),("content",0,"image_base64")]:
            try:
                r=resp
                for key in path: r = r[key] if isinstance(key,int) else getattr(r,key)
                if isinstance(r,str): b64=r; break
            except Exception: continue
        if not b64: st.error("Could not extract image from API response."); return None
        out_path=out_dir / f"{name}.png"; return pil_save_image(b64, out_path, (720,1280))
    except Exception as e:
        st.error(f"Reference image generation failed: {e}"); return None
...
def generate_single(client: OpenAI, prompt: str, size: str, seconds: int, references: List[Path] | None, model: str) -> Optional[Path]:
    ...
    if references: kwargs["input_reference"]=references[0]  # type: ignore
...
def generate_and_remix(client: OpenAI, base_prompt: str, remix_prompts: List[str], size: str, seconds: int, references: List[Path] | None, model: str) -> List[Optional[Path]]:
    ...
    if references: base_kwargs["input_reference"]=references[0]  # type: ignore
    ...
    for i, rprompt in enumerate(remix_prompts, start=1):
        ...
        outputs.append(out_path); current=remix_job
    return outputs
...
def generate_multiple(client: OpenAI, jobs: List[GenJob], size: str, seconds: int, model: str, concurrent_workers: int = 2) -> List[GenJob]:
    ...
    def worker(job: GenJob):
        ...
        if job.references: kwargs["input_reference"]=job.references[0]  # type: ignore
        try:
            vid=_video_create(client, kwargs)
        ...
        except Exception: return job.idx, None
    with ThreadPoolExecutor(max_workers=max(1,int(concurrent_workers))) as ex:
        futures=[ex.submit(worker, job) for job in jobs]
        for fut in as_completed(futures):
            try: idx, path=fut.result(); jobs[idx].result_path=path
            except Exception as e: st.error(f"A job failed: {e}")
    return jobs
...
with st.sidebar:
    ...
    strict_durations=st.checkbox("Strict supported durations", value=True, help="When on, snaps to supported values to avoid API errors.")
    ...
    st.session_state.budget_limit=st.number_input("Max budget for this session ($)", min_value=1.0, value=float(st.session_state.budget_limit), step=1.0, disabled=not st.session_state.budget_enabled)
    ...
    if st.button("Reset spend counter"):
        st.session_state.spent_usd=0.0; st.session_state.counted_ids=set(); st.success("Spend counter reset.")

    st.subheader("Outputs")
    out_root=st.text_input("Output folder", value=str(DEFAULT_OUTPUT_DIR))
    if "output_dir" not in st.session_state: st.session_state.output_dir=ensure_dir(Path(out_root)/time.strftime("%Y%m%d_%H%M%S"))
    colX, colY, colZ = st.columns([1,1,1])
    with colX:
        if st.button("New session folder"): st.session_state.output_dir=ensure_dir(Path(out_root)/time.strftime("%Y%m%d_%H%M%S"))
    with colY:
        if st.button("Zip session"):
            try:
                base=st.session_state.output_dir.with_suffix(""); zip_path=shutil.make_archive(base.as_posix(), 'zip', st.session_state.output_dir.as_posix())
                z=Path(zip_path)
                if z.exists(): st.success(f"Created: {z.name}"); st.download_button("Download ZIP", data=z.read_bytes(), file_name=z.name, mime="application/zip")
            except Exception as e: st.error(f"Zip failed: {e}")
    with colZ:
        if st.button("Clear session folder"):
            try: shutil.rmtree(st.session_state.output_dir); st.session_state.output_dir=ensure_dir(Path(out_root)/time.strftime("%Y%m%d_%H%M%S")); st.success("Cleared session outputs.")
            except Exception as e: st.error(f"Clear failed: {e}")
...
with tabs[0]:
    ...
    with colB:
        ...
        if client and st.button("Generate (Single)"):
            if st.session_state.budget_enabled and (spent + est_cost) > st.session_state.budget_limit: st.stop()
            refs: List[Path]=[]
            if gen_refs_from_prompt.strip():
                ...
                if out_img: refs.append(out_img); st.image(str(out_img), caption="Generated reference", use_container_width=True)
            if ref_uploaded:
                refs_dir=ensure_dir(st.session_state.output_dir/"refs")
                for i,uf in enumerate(ref_uploaded, start=1):
                    p=refs_dir/f"upload_{i}.png"; write_bytes(p, uf.getvalue()); refs.append(p)
                if refs: st.success(f"Loaded {len(refs)} reference image(s)."); st.image([str(x) for x in refs], caption=[x.name for x in refs])
            final_prompt=user_prompt
            enhanced_txt=None
            if use_enhance and user_prompt.strip():
                enhanced_txt=enhance_prompt(client, user_prompt, enhance_style); final_prompt=enhanced_txt
            if not user_prompt.strip() and not gen_refs_from_prompt.strip():
                ...
                try:
                    ...
                    if out_path and out_path.exists():
                        st.success("Done."); st.video(str(out_path))
                        st.download_button("Download video", data=out_path.read_bytes(), file_name=out_path.name, mime="video/mp4")
                        if enhanced_txt: st.markdown("**Enhanced prompt**"); st.code(enhanced_txt)
...
with tabs[1]:
    ...
    m_est_cost=estimate_cost(int(n), seconds, model); m_calls_per_job=1 + (1 if use_enhance else 0); m_total_calls=int(n)*m_calls_per_job
    ...
    if client and st.button("Generate All"):
        if st.session_state.budget_enabled and (spent + m_est_cost) > st.session_state.budget_limit: st.stop()
        refs: List[Path]=[]
        if m_gen_ref_prompt.strip():
            ...
            if out_img: refs.append(out_img); st.image(str(out_img), caption="Generated shared reference", use_container_width=True)
        if m_ref_uploaded:
            refs_dir=ensure_dir(st.session_state.output_dir/"refs")
            ...
            try:
                ...
                for job in sorted(jobs, key=lambda j: j.idx):
                    with st.expander(f"Result #{job.idx+1}"):
                        ...
                        if job.enhanced_prompt: st.markdown("**Enhanced prompt**"); st.code(job.enhanced_prompt)
            except Exception as e: st.exception(e)
...
with tabs[2]:
    ...
    r_videos=1+int(remix_count); r_cost=estimate_cost(r_videos, seconds, model)
    ...
    if client and st.button("Generate Remix Sequence"):
        if st.session_state.budget_enabled and (spent + r_cost) > st.session_state.budget_limit: st.stop()
        if not base_prompt.strip():
            ...
            if rr_gen_refs_from_prompt.strip():
                ...
                if out_img: refs.append(out_img); st.image(str(out_img), caption="Generated reference for base", use_container_width=True)
            if rr_ref_uploaded:
                refs_dir=ensure_dir(st.session_state.output_dir/"refs")
                for i,uf in enumerate(rr_ref_uploaded, start=1):
                    p=refs_dir/f"remix_upload_{i}.png"; write_bytes(p, uf.getvalue()); refs.append(p)
            ...
            try:
                ...
                for i,p in enumerate(valid_paths):
                    with st.expander(f"Clip {i} ({p.name})"):
                        st.video(str(p)); st.download_button("Download video", data=p.read_bytes(), file_name=p.name, mime="video/mp4")
                if len(valid_paths)>=2 and check_ffmpeg():
                    st.divider(); stitch_name=st.text_input("Stitched file name", value="sequence.mp4")
                    if st.button("Stitch all clips"):
                        outp=stitch_videos(valid_paths, st.session_state.output_dir/stitch_name)
                        if outp and outp.exists(): st.success(f"Stitched: {outp.name}"); st.video(str(outp)); st.download_button("Download stitched video", data=outp.read_bytes(), file_name=outp.name, mime="video/mp4")
            except Exception as e: st.exception(e)
...
with tabs[3]:
    ...
    with colQ2:
        if st.button("Clear queue"): st.session_state.batch_jobs=[]; st.success("Cleared.")
    ...
    with colBq1: batch_enhance=st.checkbox("Enhance prompts in batch", value=False, key="batch_enhance")
    with colBq2: batch_workers=st.number_input("Workers (internally; limiter gates calls)", min_value=1, max_value=8, value=1, step=1)
    if client and st.button("Start queue run"):
        ...
        if b_ref_uploaded:
            ...
            for i,uf in enumerate(b_ref_uploaded, start=1):
                p=refs_dir/f"batch_upload_{i}.png"; write_bytes(p, uf.getvalue()); refs.append(p)
        completed=0
        for job in st.session_state.batch_jobs:
            if job["status"] not in ("queued","error"): continue
            job["status"]="running"; st.write(f"Running: {job['prompt'][:80]}{'...' if len(job['prompt'])>80 else ''}")
            ...
            if st.session_state.budget_enabled and (st.session_state.spent_usd + j_est) > st.session_state.budget_limit:
                job["status"]="error"; st.error("Budget cap reached. Halting queue."); break
            ...
            try:
                ...
                if out_path and out_path.exists():
                    job["status"]="done"; job["path"]=out_path; st.success(f"Done: {out_path.name}"); completed+=1
                else:
                    job["status"]="error"; st.error("Job failed (no output).")
            except Exception as e:
                job["status"]="error"; st.exception(e)
    ...
    if videos:
        ...
        for v in videos:
            with st.expander(v.name):
                st.video(str(v)); st.download_button("Download", data=v.read_bytes(), file_name=v.name, mime="video/mp4")
    else: st.info("No videos yet.")
    if images:
        st.write("Reference Images"); st.image([str(x) for x in images], caption=[x.name for x in images])
        for img in images: st.download_button(f"Download {img.name}", data=img.read_bytes(), file_name=img.name)
    else: st.info("No references yet.")
